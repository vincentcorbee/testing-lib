import { describe, test, page, screen, user, expect } from '../../../dist/index.js';
...
describe('Backstage membership registration sanity', () => {
  describe('Registratie overzicht', () => {
    ...
    test('should be able to filter registrations', async () => {
      ...
      await user.type(await screen.getByLabel('Zoekterm'), 'test');
      await user.selectOptions(await screen.getByLabel('Registratie type'), 'AUTHOR');
      await user.selectOptions(await screen.getByLabel('Naam type'), 'PERSONAL');
      await user.type(await screen.getByLabel('Transactie id'), '12345');
      await user.selectOptions(await screen.getByLabel('Volgorde'), 'DESC');
