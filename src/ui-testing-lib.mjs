  ...
  return performAction((resolve, reject) => {
    ...
    if (!element)
      throw new AssertionError({
        name: 'dispatchEvent',
        expected: `Element with ${selectorOrElement}`,
        actual: element,
        pass: false,
        message: `${selectorOrElement} not found`,
      });
...
function getBySelector(selector) {
  return performAction((resolve) => {
    ...
    if (!element)
      throw new AssertionError({
        name: 'getBySelector',
        expected: `Element with ${selector}`,
        actual: element,
        pass: false,
...
function getByText(text, options = { parent: '*', index: 0 }) {
  const { parent = '*', index = 0 } = typeof options === 'string' ? { parent: options } : options || {};

  return performAction((resolve) => {
    const result = document.evaluate(
      `//${parent}[contains(text(),'${text}')]`,
      document,
      null,
